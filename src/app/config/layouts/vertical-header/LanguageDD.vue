...
const unwatch = watch(
  () => manualStore.LanguageList,
  (val) => {
    if (val) {
      if (!localStorage.getItem("lang")) {
        const lang = manualStore.LanguageList.find((el) => el.code === "uz_latn");
        if (lang) {
          ChangeLanguage(lang);
        }
      }
  ...
  <v-menu :close-on-content-click="false" location="bottom">
    <template v-slot:activator="{ props }">
      <v-btn icon variant="text" color="primary" v-bind="props">
        <v-avatar size="36" color="primary">
          <template v-for="(item, _) in manualStore.LanguageList">
            <span class="text-h5" v-if="item.code === locale">{{ item.shortName }}</span>
          </template>
        </v-avatar>
    ...
    <v-sheet rounded="md" width="200" elevation="10">
      <v-list class="theme-list">
        <v-list-item
          v-for="(item, index) in manualStore.LanguageList"
          :key="index"
          color="primary"
