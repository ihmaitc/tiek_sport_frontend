...
const SectorList = ref<ISelectList[]>([]);
...
const save = async (values: SubmitEventPromise) => {
  ...
  if (valid) {
    ...
    MfyService.Update(data.value)
      .then(() => {
        notify({
          text: i18n.global.t("SaveSuccess"),
          type: "success"
        });
        router.push({ name: "Mfy" });
      })
      .catch((err) => {
        setError(err);
      })
  ...
  <ui-parent-card>
    <v-form v-if="data" @submit.prevent="save">
      <v-row>
        ...
        <v-col lg="3" cols="12">
          <form-select :list="SectorList" v-model:model-value="data.sectorId" :label="$t('Sector')" required> </form-select>
        </v-col>
        <v-col lg="3" cols="12">
